...
def balanced_cross_entropy(y_true_cls, y_pred_cls):
    ...
    a = y_true_cls.shape[3]*y_true_cls.shape[2]
    beta = 1 - torch.sum(y_true_cls,[1,2,3])/a

    b=torch.sum(y_true_cls * torch.log(y_pred_cls),[1,2,3])
    c=torch.sum((1-y_true_cls)*torch.log(1-y_pred_cls),[1,2,3])
    d=-beta*b
    e=(1-beta)*c
    """
        a = numpy.prod(y_true_cls.shape)
        beta = 1 - torch.sum(y_true_cls)/a

        b=torch.sum(y_true_cls * torch.log(y_pred_cls))
        c=torch.sum((1-y_true_cls)*torch.log(1-y_pred_cls))
        d=-beta*b
        e=(1-beta)*c
